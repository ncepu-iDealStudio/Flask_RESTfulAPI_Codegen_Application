...
from app.utils.checkTable import CheckTable
...
class SQLHandler:
    ...
    @classmethod
    def connect_sql_link(cls, dialect, username, password, host, port, database):
        """
            数据库连接
            :param dialect: 数据库种类
            :param username: 用户名
    ...
    @classmethod
    def generate_tables_information(cls) -> dict:
        """返回所有表、字段信息（前端用）
        ...
        table_dict, invalid_tables = CheckTable.main(cls.metadata, cls.views_names)
    ...
    @classmethod
    def generate_views_information(cls) -> dict:
        """
        返回所有视图、字段信息（前端用）
        :return code: 布尔型，True表示连接成功，False表示连接失败
        :return message: 返回信息
        ...
        table_dict = CheckTable.main(cls.metadata, cls.views_names, view=True)
